...
class SGL(AbstractRecommender):
    def __init__(self, config):
        ...
        # 找出購買次數少的前 10% (Tail Users) 的門檻
        threshold = np.percentile(user_degrees, 10)
        print(f"[DEBUG] User degree 10th percentile threshold: {threshold}")

        # 預設為 0.5 (Head Users)
        self.user_temps = np.full(self.num_users, 0.5, dtype=np.float32)
        
        # [動作] 針對冷門用戶 (Tail Users, 前 10%) 給予較低溫度 (0.1)，強化其凝聚力
        # Group B Tail: 購買次數 <= threshold -> 0.1
        self.user_temps[user_degrees <= threshold] = 0.1
        
        # [動作] 針對目標群體 Group A 同樣給予低溫度 (0.1)，強制極化向量空間
        # Group A: Force to 0.1 (Solidification)
